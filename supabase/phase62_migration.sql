-- Phase 62 (2026-09-22, Sandra: "if there are manual time entries that
-- are in pending status, let's allow the assignee or requestor to
-- delete or make changes with the manual time entry log only. Should
-- apply to both project and non project manual entries only. Anything
-- after approved status will be admin corrections only") -- self-service
-- edit/delete for a still-pending manual entry, scoped tightly:
--
--   - source = 'manual' only. Timer entries already have their own
--     "fix it before it's final" step (confirm_time_entry lets the
--     person adjust times when confirming a pending_confirm entry), so
--     they don't need this.
--   - status = 'pending_approval' only. The moment it's approved (or
--     confirmed, for a timer entry), only correct_time_entry (Full
--     Access) can touch it -- unchanged from before.
--   - person_id (the assignee/logger) OR requested_by (whoever
--     submitted it, which can differ when Full Access logs on someone
--     else's behalf) may edit or delete -- mirrors the pair already used
--     by can_decide_time_entry's ownership checks.
--   - Applies to both project-task manual entries and non-project manual
--     entries (phase59) -- same functions handle both, branching on
--     whether task_id or activity_type_id is set.
...
begin
  ...
  if v_source <> 'manual' or v_status <> 'pending_approval' then
    raise exception 'only a still-pending manual entry can be edited -- once it''s decided, only a correction can change it';
  end if;
...
begin
  ...
  if v_source <> 'manual' or v_status <> 'pending_approval' then
    raise exception 'only a still-pending manual entry can be deleted -- once it''s decided, it stays on record';
  end if;
  if not (v_person = my_person_id() or v_requested_by = my_person_id() or my_access_level() = 'full') then
